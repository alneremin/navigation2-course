...
def generate_launch_description():
	...
	declare_bridge_file_path_cmd = DeclareLaunchArgument(
		...
		default_value=os.path.join(get_package_share_directory('twike_gazebo'), 'config', 'bridge.yaml'),
		description='Bridge configuration'
	)

	gazebo_cmd = IncludeLaunchDescription(
		os.path.join(get_package_share_directory("twike_gazebo"), "launch", "gazebo.launch.py"),
		launch_arguments={
			'world': world_path,
			'verbose': verbose
		}.items()
	...
	spawn_cmd = IncludeLaunchDescription(
		os.path.join(get_package_share_directory("twike_gazebo"), "launch", "spawn.launch.py"),
		launch_arguments={
			'namespace': namespace,
